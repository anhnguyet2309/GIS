...
class HomeController {
  ...
  //Tng k Huyen
  nongtrai(req, res) {
    db.Runquery(`select  maintoa.ID,maintoa.tenHuyen,maintoa.Poly,maintoa.Poly.STArea() As DienTich, temp.SoTrangTrai
    from Huyen  maintoa  join (select dt.ID,SUM(CAST(dt.Poly.STContains(pt.Poly.STPointN(pt.Poly.STNumPoints() / 2)) AS INT)) as SoTrangTrai from VungTrangTrai pt, Huyen dt group by dt.ID) temp
  ...
  khoang_cach(req,res){
    ...
    SELECT TOP (1) H.ID,H.Poly, @g.STDistance(trongtam.Centroid) as KC
  ...
  find_trangtrai(req,res){
    ...
    DECLARE @g geography; 
    SET @g = geography::Point(${x},${y}, 4326);
    SELECT TOP (1) H.nameVung, H.ID,H.Poly, @g.STDistance(trongtam.Centroid) as KC
